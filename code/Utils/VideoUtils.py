...
class VideoReader:
    ...
    def __init__(self, DatasetPath, question, vidId, loadAllFrames=True,
                 maxFrames=math.inf):
        self.vidPath = '{}/Q{}/{:d}.avi'.format(DatasetPath, question, vidId)
        self.flowFol = '{}/Q{}/{:d}_flows'.format(DatasetPath, question, vidId)
        self.video = cv2.VideoCapture(self.vidPath)
        self.fps = self.video.get(cv2.CAP_PROP_FPS)
        self.width = int(self.video.get(cv2.CAP_PROP_FRAME_WIDTH))
        self.height = int(self.video.get(cv2.CAP_PROP_FRAME_HEIGHT))
        self.nrFrames = min(int(self.video.get(cv2.CAP_PROP_FRAME_COUNT)),
                            maxFrames)
        if loadAllFrames:
    ...
    def extractFrames(self):
        frames = np.zeros((self.nrFrames, self.height, self.width, 3),
                          dtype='uint8')
    ...
    def getFlow(self, frameIdx):
        path = '{}/{:04d}.flo'.format(self.flowFol, frameIdx)
